...
public class ModelConfigHelper {
    ...
    public boolean testConnection(ChatModelConfig modelConfig) {
        try {
            if (modelConfig == null || StringUtils.isBlank(modelConfig.getBaseUrl())) {
                return false;
            }
            String response = llmService.collectFluxToStringSafe(llmService.callUser("Hi there"));
    ...
    public ChatModelConfig getChatModelConfig(ChatApp chatApp) {
