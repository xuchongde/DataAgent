...
import com.alibaba.cloud.ai.headless.chat.knowledge.EmbeddingResult;
import com.alibaba.cloud.ai.headless.chat.knowledge.MetaEmbeddingService;
...
import static com.alibaba.cloud.ai.headless.chat.mapper.MapperConfig.*;
...
public class EmbeddingMatchStrategy extends BatchMatchStrategy<EmbeddingResult> {
    ...
    public List<EmbeddingResult> detectByBatch(ChatQueryContext chatQueryContext,
        ...
        if (useLlm) {
            ...
            String promptText = generatePrompt(chatQueryContext, results);
     ...
     * 生成提示词
     */
    private String generatePrompt(ChatQueryContext chatQueryContext,Set<EmbeddingResult> results) {
        Map<String, Object> variable = new HashMap<>();
        variable.put("userText", chatQueryContext.getRequest().getQueryText());
        variable.put("retrievedInfo", JSONObject.toJSONString(results));

        PromptTemplate promptTemplate = new PromptTemplate(LLM_FILTER_PROMPT);
